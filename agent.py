...
TOOLS_PROMPT = (
    ...
    "如果用户一句话里有多项控制（常见连接词：然后、再、并且、顺便、同时、和），"
    "必须按执行顺序输出 JSON 数组，绝不能只执行最后一项、也不能漏掉前面的动作。"
    "例如'把音量调到50%，播放音乐花海'："
    "[{\"action\":\"set_volume\",\"value\":50},{\"action\":\"play_music\",\"name\":\"花海\"}]。"
    "例如'音量调到15%，然后停止播放音乐'："
    "[{\"action\":\"set_volume\",\"value\":15},{\"action\":\"stop_media\"}]。\n"
    ...
    "这种请求里的 print 必须带 content，content 必须与 say 的 text 完全相同；"
    "绝对不要输出没有 content 的 {\"action\":\"print\"}。"
    "例如：[{\"action\":\"say\",\"text\":\"为什么电脑喜欢喝茶？因为它怕死机。\"},"
    "{\"action\":\"print\",\"content\":\"为什么电脑喜欢喝茶？因为它怕死机。\"}]。"
    "用户说'再打印一遍/重新打印'时，根据上面的对话历史判断要重复打印的文字，"
    "并输出 {\"action\":\"print\",\"content\":\"要重复打印的完整文字\"}；"
    "如果无法从历史里确定具体文字，正常回复一句话询问用户要打印哪一段，不要调用工具。\n\n"
    "8. 游戏（FCEUX/NES）：\n"
    ...
    "当用户说'退下'、'你退下吧'、'退下退下'、'不要说话了'、'闭嘴'、'安静'、'别说话'、"
    "'别理我'、'让我安静'、'没事了'、'不聊了'，或任何表达让你回去待命/结束对话的话时，"
    "你的整条回复必须就是 {\"action\": \"enter_wait_wake\"} 这串 JSON，前后不能有任何文字。"
    "特别注意：绝对不要自己用文字说「好的，我先退下了」「好的我退下了」之类的告别语——"
    "那句话由系统在执行工具后自动播报，你只要输出 JSON；自己说出来就等于没真正退下，是错误的。\n"
    ...
    "'太吵了小声点'、'音量调到50'；以及让你退下/安静），就必须直接输出对应工具的纯 JSON，"
    "绝对不能用文字说'我做不到'、'我没法调音量'之类的话。\n"
    "最关键的一条：当你判断要用某个工具时，整条回复就只有那串 JSON，"
    "绝对不要顺带把执行后才该说的确认话也写出来（例如要退下就只输出 enter_wait_wake 的 JSON，"
    "不要写「好的，我退下了」；要调音量就只输出 set_volume 的 JSON，不要写「音量调好了」）。"
    "那些确认话都由系统在工具执行后自动播报；你一旦自己用文字说出来，工具就没被真正调用，是错误的。\n"
